...
fn main() {
    ...
    while idx < vm.output.len() {
        let c = (vm.output[idx] as isize, vm.output[idx+1] as isize);
        let t = vm.output[idx+2];
        
        tiles.insert(c, t as usize);

        idx += 3;
    }

    println!("#1 {}", tiles.iter().filter(|(_, &t)| t == 2).count());

    // part 2
    let mut tiles: HashMap<(isize, isize), usize> = HashMap::new();
    let mut vm = IntCode::new(&program, &[1]);
    vm.program[0] = 2;
    ...
    let score = loop {
        ...
        // find ball position
        let ball_pos = tiles.iter().find(|(_, &t)| t == 4).unwrap();
        // find curser position
        let paddle_pos = tiles.iter().find(|(_, &t)| t == 3).unwrap();

        let mut input = 0;

        if ball_pos.0.0 > paddle_pos.0.0 {
            input = 1;
        }
        if ball_pos.0.0 < paddle_pos.0.0 {
            input = -1;
        ...
        // println!("ball: {:?} cursor: {:?}, decision: {:?}", ball_pos, paddle_pos, input);

        vm.input.clear();
        vm.input.push(input);

        if vm.is_halted || tiles.iter().filter(|(_, &t)| t == 2).count() == 0 {
            break tiles.get(&(-1, 0)).unwrap()
        }
    };
...
fn draw(tiles: &HashMap<(isize, isize), usize>) {
    for y in 0..=20 {
        for x in 0..=35 {
            let c = match tiles.get(&(x, y)).unwrap() {
                0 => ' ',
                1 => '#',
                2 => 'X',
                3 => '-',
    ...
    println!("Score: {}", tiles.get(&(-1, 0)).unwrap());
    println!();
}
